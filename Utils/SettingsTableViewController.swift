...
class SettingsTableViewController: UITableViewController {
    ...
    func didSelectVideoCodecRow(indexPath: IndexPath) {
        ...
        for codec in selectionArray {
            ...
            if (settings.videoCodec == codec) {
                ...
                if settings.videoCodec!.isKind(of: Vp8Codec.self) {
                    if codec.isKind(of: Vp8Codec.self) {
                        if ((codec as! Vp8Codec).isSimulcast == (settings.videoCodec as! Vp8Codec).isSimulcast) {
                            selectedButton = selectionButton;
                        }
                    }
                } else {
    ...
    func getDisplayStrForVideoCodec(codec: VideoCodec) -> String {
        var str = codec.name
        if codec.isKind(of: Vp8Codec.self) {
            let vp8Codec = codec as! Vp8Codec
